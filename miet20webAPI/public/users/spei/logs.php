...
if ($fecha_desde !== '') {
  try {
    $query['fechaDesde'] = spei_normalize_date_input($fecha_desde);
...
if ($fecha_hasta !== '') {
  try {
    $query['fechaHasta'] = spei_normalize_date_input($fecha_hasta);
  } catch (RuntimeException $exception) {
    $filterError = $exception->getMessage();
  }
}
...
try {
  $response = spei_call_api('GET', '/devices/prestamos', null, ['query' => $query]);
  $data = $response['data'] ?? [];
  ...
  if (is_array($data)) {
    foreach ($data as $item) {
      ...
      $loanRows[] = [
        ...
        'fecha_devolucion' => spei_format_display_date($item['fecha_devolucion'] ?? null),
        'hora_devolucion' => trim((string) ($item['hora_devolucion'] ?? ''))
      ];
    }
  }
} catch (RuntimeException $exception) {
...
<body class="bg-gray-100 min-h-screen flex relative">
  ...
  <main id="content" class="flex-1 p-10">
    ...
    <?php endforeach; ?>
    <?php if ($filterError): ?>
      <div class="mb-4 px-4 py-3 rounded border bg-yellow-100 border-yellow-200 text-yellow-800">
      ...
      <table class="min-w-full">
        ...
        <tbody class="text-gray-700 divide-y text-sm">
          <?php foreach ($loanRows as $loan): ?>
            <tr>
              <td class="py-2 px-4"><?= (int) $loan['id'] ?></td>
              <td class="py-2 px-4"><?= htmlspecialchars($loan['codigo']) ?></td>
              <td class="py-2 px-4"><?= htmlspecialchars($loan['alumno']) ?></td>
              <td class="py-2 px-4"><?= htmlspecialchars($loan['curso']) ?></td>
              <td class="py-2 px-4"><?= htmlspecialchars($loan['tutor']) ?></td>
              <td class="py-2 px-4"><?= htmlspecialchars($loan['fecha_prestamo']) ?></td>
              <td class="py-2 px-4"><?= htmlspecialchars($loan['hora_prestamo']) ?></td>
              <td class="py-2 px-4"><?= $loan['fecha_devolucion'] !== '' ? htmlspecialchars($loan['fecha_devolucion']) : '-' ?></td>
              <td class="py-2 px-4"><?= $loan['hora_devolucion'] !== '' ? htmlspecialchars($loan['hora_devolucion']) : '-' ?></td>
            </tr>
          <?php endforeach; ?>
        </tbody>
      </table>
