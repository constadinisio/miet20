...
// Asignaciones solo para el cargo elegido
$cargo_id = $_GET['cargo_id'] ?? null;
$asignaciones = [];
if ($cargo_id) {
    $stmt = $conexion->prepare("SELECT c.id AS curso_id, m.id AS materia_id, c.anio, c.division, m.nombre AS materia
        FROM cargos_cursos cc
        JOIN cursos c ON cc.curso_id = c.id
        JOIN cargos_materias cm ON cc.cargo_id = cm.cargo_id
...
$todosLosCargos = [];
$sql = "SELECT c.id, c.codigo_cargo, c.tipo, u.apellido, u.nombre AS docente_nombre
        FROM cargos c
...
<!-- DRAWER:HTML END -->
            ...
            <div class="mb-4 px-4 py-3 rounded-xl bg-green-100 border border-green-400 text-green-800">
                ...
                switch ($_GET['ok']) {
                    ...
                    case 'estado_actualizado':
                        ...
                        break;
                    case 'asignada':
                        echo '✅ Materia asignada al cargo.';
                        break;
                    case 'eliminada':
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const select = document.getElementById('select-cargos');
