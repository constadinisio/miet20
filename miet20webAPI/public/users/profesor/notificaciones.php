...
// Cursos asignados
$cursos = [];
$sql = "SELECT cu.id, cu.anio, cu.division
        FROM cargos cg
        ...
        WHERE cg.docente_id = ?
        GROUP BY cu.id, cu.anio, cu.division
...
if (!empty($cursos)) {
    ...
    $q = $conexion->query("SELECT u.id, u.nombre, u.apellido
                           ...
                           WHERE ac.curso_id IN ($curso_ids) AND ac.estado = 'activo' AND u.rol = 4
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $_POST['csrf'] === $csrf) {
    ...
    if ($tipo === 'INDIVIDUAL') {
        ...
        foreach ($destinos as $curso_id) {
            ...
            $q = $conexion->query("SELECT alumno_id FROM alumno_curso WHERE curso_id = $curso_id AND estado = 'activo'");
