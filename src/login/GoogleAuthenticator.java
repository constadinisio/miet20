...
public class GoogleAuthenticator {
    ...
    public UserSession authenticateUser() throws IOException {
        ...
        if (nombre == null) nombre = userEmail.split("@")[0];
    ...
    try {
        ...
        if (rs.next()) {
            // Usuario existe - obtener rol y actualizar nombre/apellido
            int rol = rs.getInt("rol");
            
            String updateQuery = "UPDATE usuarios SET nombre = ?, apellido = ? WHERE mail = ?";
            ...
            String insertQuery = "INSERT INTO usuarios (nombre, apellido, mail, rol, status, contrasena) " +
                               "VALUES (?, ?, ?, 0, 1, 'default_password')";
