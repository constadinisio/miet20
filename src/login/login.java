    ...
    private void updateUserInterface(JFrame form, UserSession session) {
        try {
            PreparedStatement stmt = conect.prepareStatement(
                "SELECT u.nombre, u.apellido, c.anio, c.division " +
                "FROM usuarios u " +
                "LEFT JOIN alumno_curso ac ON u.id_alumno = ac.alumno_id " +
                "LEFT JOIN cursos c ON ac.curso_id = c.id " +
                "WHERE u.mail = ? AND ac.estado = 'activo'"
            );
            ...
            if (rs.next()) {
                ...
                if (form instanceof alumnos) {
                    ((alumnos) form).updateLabels(nombreCompleto, String.valueOf(session.getRol()), cursoDiv);
                }
            }
